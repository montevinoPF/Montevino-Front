...
const PlatosYCategoriasView = () => {
  ...
  const [todasLasBebidas, setTodasLasBebidas] = useState<IProduct[]>([]); // <-- todas
  const [bebidas, setBebidas] = useState<IProduct[]>([]); // <-- página actual
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const [todosLosPlatos, setTodosLosPlatos] = useState<IProduct[]>([]); // <-- todos
  const [platos, setPlatos] = useState<IProduct[]>([]); // <-- página actual

  const [currentPagePlatos, setCurrentPagePlatos] = useState(1);
  const [hasNextPagePlatos, setHasNextPagePlatos] = useState(false);

  const [currentPageBebidas, setCurrentPageBebidas] = useState(1);
  const [hasNextPageBebidas, setHasNextPageBebidas] = useState(false);
  ...
  useEffect(() => {
    if (!isAuthReady || !userData || userData.user.role !== "ADMIN") return;
    const fetchPlatos = async () => {
      try {
        const data = await getTodosLosPlatos();
  ...
  // Cargar bebidas UNA SOLA VEZ
  useEffect(() => {
    if (!isAuthReady || !userData || userData.user.role !== "ADMIN") return;
    const fetchBebidas = async () => {
      try {
        const data = await getBebidas();
        setTodasLasBebidas(data);
      } catch (error) {
        console.error(error);
      }
    };
    fetchBebidas();
  }, [isAuthReady, userData]); // <-- sin currentPageBebidas

  // Paginar bebidas en el frontend cuando cambia la página
  useEffect(() => {
    const start = (currentPageBebidas - 1) * ITEMS_PER_PAGE;
    const end = start + ITEMS_PER_PAGE;
    const paginadas = todasLasBebidas.slice(start, end);
    setBebidas(paginadas);
    setHasNextPageBebidas(end < todasLasBebidas.length);
  }, [currentPageBebidas, todasLasBebidas]);

  // Paginar platos en el frontend cuando cambia la página
  useEffect(() => {
    const start = (currentPagePlatos - 1) * ITEMS_PER_PAGE;
    const end = start + ITEMS_PER_PAGE;
    const paginados = todosLosPlatos.slice(start, end);
    setPlatos(paginados);
    setHasNextPagePlatos(end < todosLosPlatos.length);
  }, [currentPagePlatos, todosLosPlatos]);

  // Cargar categorías y verificar admin
  useEffect(() => {
              ...
              >
                ...
                <td className="flex gap-3 py-4">
                  <button
                    ...
                    onClick={() =>
                      router.push(`/admin/productos/editar-producto/${p.id}`)
                    }
                  >
                    Editar
      ...
      <div className="flex items-center justify-between px-6 py-4 border-t border-gray-200">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 transition-colors border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            <ChevronLeft size={18} />
          </button>
      ...
      <div className="h-full mt-20 w-full bg-[#F6E3D9] flex">
        ...
        <div className="flex-col items-center justify-center flex-1 mx-10 mb-10">
          ...
          {loading ? (
            ...
            <ProductTable
              items={platos}
              currentPage={currentPagePlatos}
              hasNextPage={hasNextPagePlatos}
              onPrev={() => setCurrentPagePlatos((p) => Math.max(p - 1, 1))}
              onNext={() => setCurrentPagePlatos((p) => p + 1)}
            />
          )}
          ...
          {loading ? (
            <p className="py-10 text-xl text-center">Cargando bebidas...</p>
          ) : (
            <ProductTable
              items={bebidas}
              currentPage={currentPageBebidas}
              hasNextPage={hasNextPageBebidas}
              onPrev={() => setCurrentPageBebidas((p) => Math.max(p - 1, 1))}
              onNext={() => setCurrentPageBebidas((p) => p + 1)}
            />
          )}
          <div className="flex justify-center mt-6 mb-10">
            <Link href="/admin/platos/crear-producto">
              <button className="relative overflow-hidden py-3 w-50 bg-gradient-to-r from-[#3d0c07] to-[#56070C] text-white font-semibold rounded-md shadow-lg transition duration-300 group cursor-pointer">
                Crear Bebida
                <span className="absolute inset-0 transition-transform -translate-x-full bg-gradient-to-r from-transparent via-white/40 to-transparent group-hover:translate-x-full duration-1500"></span>
              </button>
